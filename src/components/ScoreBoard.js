...
import LiveScreeningScoreboard from "./ui/live-scoreboard";
...
const ScoreBoard = () => {
  ...
  const currentBowlOver = Number(bowlers.find((b) => b?.id === bowler?.id)?.over);
  const currentBowlRun  = Number(bowlers.find((b) => b?.id === bowler?.id)?.run)

  return (
    <div className="container">
      <LiveScreeningScoreboard
        team1={team1}
        team2={team2}
        ...
        batter1={batter1}
        batter2={batter2}
        bowlerName={inputBowler}
        target={target}
        rrr={isNaN(rrr) ? 0 : rrr}
        winningMessage={winningMessage}
        currentBowlerOvers={{
          over: `${isNaN(currentBowlOver) ? 0 : currentBowlOver}.${ballCount}`,
          run: (isNaN(currentBowlRun) ? 0 : currentBowlRun) + runsByOver
        }}
      />
